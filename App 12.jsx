...
// audio baked into the app (committed to the repo as /blessing.mp3)
const BUILTIN_TAKE = { id: "__builtin__", name: "Blessing (built-in)" };
...
function buildGraph(ctx, dest, cfg) {
  const {
    ...
    noiseType, noiseVol,
    voiceBuf, voiceEl, voiceMode, voiceVol, pulseDepth,
    volL = 1, volR = 1,
  } = cfg;
  ...
  if (mode === "binaural") {
    ...
    const L = mk(+1, 0, volL);
    const R = mk(-1, 1, volR);
    handles.oscL = L.osc; handles.oscR = R.osc;
    handles.gainL = L.gain; handles.gainR = R.gain;
  } else {
  ...
  // ---- voice ----
  // Live playback uses a media element (iOS's native decoder plays its own
  // recordings correctly; WebAudio's decodeAudioData truncates them).
  // Offline export still uses a decoded buffer (no elements offline).
  if (voiceEl || voiceBuf) {
    let src;
    if (voiceEl) {
      src = ctx.createMediaElementSource(voiceEl);
    } else {
      src = ctx.createBufferSource();
      src.buffer = voiceBuf;
      src.loop = true;
      src.loopStart = 0;
      src.loopEnd = voiceBuf.duration;
    }
    ...
    if (voiceMode === "clear") {
      // clean narration with a transparent brick-wall limiter — allows heavy
      // boost (accessibility) without clipping distortion
    ...
    if (voiceMode !== "clear") comp.connect(master);
    if (!voiceEl) src.start();
    handles.voiceSrc = voiceEl ? null : src;
    handles.voiceGain = vGain;
  }
...
export default function App() {
  ...
  const [beat, setBeat] = useState(6);   // default: 6 Hz theta+
  const [base, setBase] = useState(113); // default: L116 / R110 carriers
  const [volume, setVolume] = useState(0.5);
  const [volL, setVolL] = useState(1); // left-ear tone level (0–1 of master)
  const [volR, setVolR] = useState(1); // right-ear tone level
  ...
  const [voiceVol, setVoiceVol] = useState(3); // 300% default — high-volume listening
  const [pulseDepth, setPulseDepth] = useState(0.35);
  const [voiceMode, setVoiceMode] = useState("clear"); // clear | unison | alternate
  const [voiceSpeed, setVoiceSpeed] = useState(1); // 1.0 = clean; any other value engages iOS's stretcher (warble)
  ...
  const voiceBufRef = useRef(null);   // decoded buffer (export only — may be truncated on iOS)
  const voiceBlobRef = useRef(null);  // the actual recording — source of truth for playback
  const voiceElRef = useRef(null);    // session media element
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      // bake-in: the bundled blessing loads itself so the app is ready to Play
      if (!voiceBlobRef.current) loadBuiltin();
    })();
  }, []);
  ...
  const start = useCallback(() => {
    stopPreview(); // never let the dry preview run under a session
    ...
    if (voiceBlobRef.current) {
      const url = URL.createObjectURL(voiceBlobRef.current);
      voiceEl = new Audio(url);
      voiceEl.loop = true;
      voiceEl.playsInline = true;
      voiceEl.playbackRate = voiceSpeed;
      voiceEl.preservesPitch = true;
      voiceEl.webkitPreservesPitch = true;
    ...
    graphRef.current = buildGraph(ctx, session, {
      ...
      noiseType, noiseVol,
      voiceEl, voiceMode, voiceVol, pulseDepth,
      volL, volR,
    });
    voiceEl?.play().catch(() => {});
  ...
  const stop = useCallback(() => {
    ...
    if (ctx && sessionGainRef.current) {
      ...
      try { voiceElRef.current?.pause(); } catch (e) {}
      setTimeout(() => {
        try {
  ...
  useEffect(() => {
    const ctx = ctxRef.current, g = graphRef.current;
    if (!ctx || !playing || !g?.gainL) return;
    g.gainL.gain.setTargetAtTime(0.9 * volL, ctx.currentTime, 0.05);
    g.gainR.gain.setTargetAtTime(0.9 * volR, ctx.currentTime, 0.05);
  }, [volL, volR, playing]);
  ...
  useEffect(() => {
    if (voiceElRef.current) {
      voiceElRef.current.playbackRate = voiceSpeed;
      voiceElRef.current.preservesPitch = true;
      voiceElRef.current.webkitPreservesPitch = true;
    }
  }, [voiceSpeed, playing]);
  ...
  // ---------- voice takes ----------
  const sniffAudioType = async (blob) => {
    try {
      const head = new Uint8Array(await blob.slice(0, 12).arrayBuffer());
      const tag = (o) => String.fromCharCode(...head.slice(o, o + 4));
      if (tag(0) === "caff") return "audio/x-caf";        // Shortcuts spoken audio
      if (tag(4) === "ftyp") return "audio/mp4";          // m4a / aac
      if (tag(0) === "RIFF") return "audio/wav";
      if (head[0] === 0xff && (head[1] & 0xe0) === 0xe0) return "audio/mpeg"; // mp3
  ...
  const loadBuiltin = async () => {
    try {
      const r = await fetch("/blessing.mp3");
      if (!r.ok) return false;
      const raw = await r.blob();
      await decodeBlob(new Blob([raw])); // typeless → format sniffer identifies mp3/m4a/caf
      setActiveTakeId(BUILTIN_TAKE.id);
      return true;
    } catch (e) { return false; }
  };

  const decodeBlob = async (blob) => {
    // retype mystery files (e.g. extension-less Shortcuts output) so iOS can play them
    if (!blob.type || blob.type === "application/octet-stream") {
      const mime = await sniffAudioType(blob);
      blob = new Blob([blob], { type: mime });
    }
    voiceBlobRef.current = blob;
    // true duration from the native player (trustworthy on iOS)
    try {
      ...
      if (isFinite(probe.duration) && probe.duration > 0) setVoiceDur(probe.duration);
      URL.revokeObjectURL(u);
    } catch (e) {}
    // best-effort decode for WAV export only
    try {
      const AC = window.AudioContext || window.webkitAudioContext;
      const tmp = new AC();
      voiceBufRef.current = await tmp.decodeAudioData(await blob.arrayBuffer());
      tmp.close();
    } catch (e) { voiceBufRef.current = null; }
    setVoiceReady(true);
  };
  ...
  const loadTake = async (id) => {
    if (id === BUILTIN_TAKE.id) { await loadBuiltin(); return; }
  ...
  const clearVoice = () => { stopPreview(); voiceBufRef.current = null; voiceBlobRef.current = null; setVoiceReady(false); setVoiceDur(0); setActiveTakeId(null); };
  ...
  const stopPreview = () => {
    try { previewRef.current?.pause(); } catch (e) {}
  ...
  const previewTake = async (id) => {
    ...
    stopPreview();
    const blob0 = id === BUILTIN_TAKE.id
      ? await fetch("/blessing.mp3").then((r) => (r.ok ? r.blob() : null)).catch(() => null)
      : await takeStore.getBlob(id);
    if (!blob0) return;
  ...
  const resetDefaults = () => {
    setMode("binaural"); setBeat(6); setBase(113); setVolume(0.5); setVolL(1); setVolR(1);
    setRampOn(false); setRampFrom(10); setRampMin(10); setTimerMin(0);
    setNoiseType("off"); setNoiseVol(0.25);
    setVoiceMode("clear"); setVoiceVol(3); setPulseDepth(0.35); setVoiceSpeed(1);
  };
  ...
  const savePreset = () => {
    ...
    if (!name) return;
    const p = { id: "p" + Date.now(), name, beat, base, mode, voiceMode, pulseDepth, noiseType, noiseVol, rampOn, rampFrom, rampMin, timerMin, volume, volL, volR };
  ...
  const applyPreset = (p) => {
    ...
    setVolL(p.volL ?? 1); setVolR(p.volR ?? 1);
  };
  ...
  const exportWav = async () => {
    ...
    try {
      ...
      if (voiceBlobRef.current && (!voiceBufRef.current || (voiceDur > 0 && voiceBufRef.current.duration < voiceDur - 1))) {
        alert("Note: this phone's decoder shortens the voice inside WAV exports (playback in-app is unaffected). The export will render with what it can decode.");
      }
    ...
    <div style={S.page}>
      ...
      <section style={S.panel}>
        <div style={S.voiceHead}>
          ...
          <span style={{ display: "flex", gap: 8 }}>
            <button className="chip" style={S.tinyBtn} onClick={resetDefaults}>↺ reset</button>
      ...
      <section style={S.panel}>
        ...
        {previewingId && (
          <button className="chip" onClick={stopPreview}
            style={{ width: "100%", marginTop: 10, padding: "13px 0", borderRadius: 10, border: "1px solid #C2564B",
              background: "#C2564B", color: "#EDE9E0", fontSize: 14, fontWeight: 600, cursor: "pointer", fontFamily: "inherit" }}>
            ◼ stop voice preview
          </button>
        )}
        ...
        <div style={S.takeList}>
          <div style={{ ...S.takeRow, borderColor: activeTakeId === BUILTIN_TAKE.id ? "#E8A34C" : "#2C2540" }}>
                <button style={S.takeName} onClick={() => loadTake(BUILTIN_TAKE.id)}>
                  {activeTakeId === BUILTIN_TAKE.id ? "▶ " : ""}★ {BUILTIN_TAKE.name}
                </button>
            <button style={{ ...S.takeDel, color: previewingId === BUILTIN_TAKE.id ? "#E8A34C" : "#9A92AC" }}
              title="listen dry" onClick={() => previewTake(BUILTIN_TAKE.id)}>
              {previewingId === BUILTIN_TAKE.id ? "◼" : "▷"}
            </button>
          </div>
          {takes.map((t) => (
        ...
        {voiceReady && (
          <>
            <div style={{ ...S.sliderHead, marginTop: 12 }}><span style={S.sliderLabel}>Voice style</span></div>
            <div style={S.chipsRowTight}>
              <Chip on={voiceMode === "clear"} onClick={() => setVoiceMode("clear")}>
                <span style={S.chipSm}>Clear — no pulse</span>
              </Chip>
              <Chip on={voiceMode === "unison"} onClick={() => setVoiceMode("unison")}>
                <span style={S.chipSm}>Pulsing throb</span>
              </Chip>
              <Chip on={voiceMode === "alternate"} onClick={() => setVoiceMode("alternate")}>
                <span style={S.chipSm}>Alternating L/R</span>
              </Chip>
            </div>
            {playing && <div style={S.hint}>Voice style changes take effect on next play.</div>}

            <Row label="Voice level" val={`${Math.round(voiceVol * 100)}%`} />
            <input type="range" min={0} max={800} value={Math.round(voiceVol * 100)} onChange={(e) => setVoiceVol(Number(e.target.value) / 100)}
              style={{ ...S.range, accentColor: "#E8A34C" }} aria-label="Voice level" />
            <Row label="Voice speed" val={voiceSpeed === 1 ? "100% · clean" : `${Math.round(voiceSpeed * 100)}%`} />
            <input type="range" min={50} max={120} value={Math.round(voiceSpeed * 100)}
              onChange={(e) => {
                const v = Number(e.target.value);
                setVoiceSpeed(v >= 94 && v <= 106 ? 1 : v / 100); // snap to clean
              }}
              style={{ ...S.range, accentColor: voiceSpeed === 1 ? "#63C2B0" : "#E8A34C" }} aria-label="Voice speed" />
            {voiceSpeed !== 1 && (
              <div style={S.hint}>Speeds other than 100% add a grainy warble (iOS's stretcher).
                For a genuinely slower clear voice, regenerate the file slow in Shortcuts and play it at 100%.</div>
            )}
            {voiceMode === "unison" && (
              <>
      ...
      <section style={S.controls}>
        ...
        {mode === "binaural" && (
          <>
            <div>
              <Row label="Left ear tone" val={`${Math.round(volL * 100)}%`} />
              <input type="range" min={0} max={100} value={Math.round(volL * 100)} onChange={(e) => setVolL(Number(e.target.value) / 100)}
                style={{ ...S.range, accentColor: "#E8A34C" }} aria-label="Left ear tone level" />
            </div>
            <div>
              <Row label="Right ear tone" val={`${Math.round(volR * 100)}%`} />
              <input type="range" min={0} max={100} value={Math.round(volR * 100)} onChange={(e) => setVolR(Number(e.target.value) / 100)}
                style={{ ...S.range, accentColor: "#63C2B0" }} aria-label="Right ear tone level" />
            </div>
          </>
        )}
      </section>
