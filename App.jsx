...
// IndexedDB for voice takes (audio blobs are too big for localStorage)
const idb = {
  open() {
  ...
  async put(take) {
    try {
      ...
      await new Promise((res, rej) => {
        const tx = db.transaction("takes", "readwrite").objectStore("takes").put(take);
  ...
  async del(id) {
    try {
      const db = await this.open();
      await new Promise((res, rej) => {
        const tx = db.transaction("takes", "readwrite").objectStore("takes").delete(id);
        tx.onsuccess = res; tx.onerror = () => rej(tx.error);
      });
    } catch (e) {}
  },
...
// iOS IndexedDB is unreliable with stored Blobs — store raw bytes + mime instead.
...
function buildGraph(ctx, dest, cfg) {
  ...
  if (voiceBuf) {
    const src = ctx.createBufferSource();
    src.buffer = voiceBuf; src.loop = true;
...
export default function App() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const t = await idb.all();
      setTakes(t.map(({ id, name, date }) => ({ id, name, date })));
  ...
  const startRecording = async () => {
    try {
      ...
      rec.onstop = async () => {
        ...
        const take = { id: "t" + Date.now(), name, date: Date.now(), mime: blob.type || "audio/mp4", buf };
        await idb.put(take);
        setTakes((p) => [{ id: take.id, name, date: take.date }, ...p]);
  ...
  const onUpload = async (e) => {
    ...
    if (f) {
      ...
      const take = { id: "t" + Date.now(), name, date: Date.now(), mime: f.type || "audio/mp4", buf };
      await idb.put(take);
      setTakes((p) => [{ id: take.id, name: take.name, date: take.date }, ...p]);
      setActiveTakeId(take.id);
    }
    e.target.value = "";
  };
  ...
  const loadTake = async (id) => {
    ...
    if (t) { await decodeBlob(takeToBlob(t)); setActiveTakeId(id); }
  };
  ...
  const previewTake = async (id) => {
    ...
    const t = all.find((x) => x.id === id);
    if (!t) return;
    const url = URL.createObjectURL(takeToBlob(t));
      ...
      <section style={S.panel}>
        ...
        {takes.length > 0 && (
          <>
            ...
            <div style={S.hint}>Takes are stored on this phone. ⬇ saves a permanent copy to Files (re-add later via upload).</div>
