...
export default function App() {
  ...
  const [voiceVol, setVoiceVol] = useState(1.2); // 1.0 = unity; boost up to 4x
  ...
  const start = useCallback(() => {
    ...
    if (voiceBufRef.current) {
      ...
      // soft limiter so a boosted voice stays clean instead of clipping
      ...
      trem.connect(vGain);
      vGain.connect(comp);
      comp.connect(gain); // voice rides the master fade + background stream
      src.start();
      ...
      <section style={S.voicePanel}>
        ...
        {voiceReady && (
          <>
            ...
            <input type="range" min={0} max={400} value={Math.round(voiceVol * 100)}
