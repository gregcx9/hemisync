  ...
  const startRecording = async () => {
    try {
      ...
      rec.onstop = async () => {
        ...
        setActiveTakeId(id);
        if (!storageOk) {
          // phone is blocking storage — push a permanent copy to Files automatically
      ...
      <section style={S.panel}>
        ...
        {!storageOk && (
          <div style={{ ...S.hint, color: "#E8A34C", border: "1px solid #4A415F", borderRadius: 8, padding: "8px 10px", marginBottom: 4 }}>
            ⚠ This phone is blocking app storage, so takes can't persist here.
            New recordings will auto-save to your Files app instead — load them
            back with ⇧ upload. Likely fixes: Settings → Apps → Safari →
            "Block All Cookies" OFF; not a Private tab; Lockdown Mode off.
          </div>
        )}
        <div style={S.btnRow}>
          <button className="chip" onClick={recording ? stopRecording : startRecording}
