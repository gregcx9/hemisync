...
function buildGraph(ctx, dest, cfg) {
  ...
  if (voiceEl || voiceBuf) {
    ...
    if (voiceMode === "clear") {
      // untouched narration — no pulse, no pan, no compressor: full fidelity
      src.connect(vGain);
...
export default function App() {
  ...
  const [voiceVol, setVoiceVol] = useState(1.5); // 150% — present above the bed with clean headroom
  ...
  const loadBuiltin = async () => {
    try {
      ...
      await decodeBlob(new Blob([blob], { type: "audio/mpeg" }));
  ...
  const resetDefaults = () => {
    setMode("binaural"); setBeat(4); setBase(200); setVolume(0.5); setVolL(1); setVolR(1);
    setRampOn(false); setRampFrom(10); setRampMin(10); setTimerMin(0);
    setNoiseType("off"); setNoiseVol(0.25);
    setVoiceMode("clear"); setVoiceVol(1.5); setPulseDepth(0.35); setVoiceSpeed(1);
  };
