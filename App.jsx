...
function buildGraph(ctx, dest, cfg) {
  ...
  if (voiceEl || voiceBuf) {
    ...
    if (voiceMode === "clear") {
      // untouched narration — no pulse, no pan; the tones alone carry the beat
      src.connect(vGain);
      vGain.connect(comp);
...
export default function App() {
  ...
  const [voiceVol, setVoiceVol] = useState(2); // default 200% — voice clearly above the bed
  ...
  const resetDefaults = () => {
    ...
    setVoiceMode("clear"); setVoiceVol(2); setPulseDepth(0.35); setVoiceSpeed(1);
  };
