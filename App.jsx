...
export default function App() {
  ...
  const startRecording = async () => {
    try {
      ...
      rec.onstop = async () => {
        ...
        // save into take library
        const name = (activeScript?.title || "Take") + " · " + new Date().toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" });
        const take = { id: "t" + Date.now(), name, date: Date.now(), blob };
        await idb.put(take);
  ...
  const onUpload = async (e) => {
    ...
    if (f) {
      await decodeBlob(f);
      const take = { id: "t" + Date.now(), name: f.name.replace(/\.[^.]+$/, ""), date: Date.now(), blob: f };
      await idb.put(take);
  ...
  const loadTake = async (id) => {
    ...
    if (t) { await decodeBlob(t.blob); setActiveTakeId(id); }
  };
  ...
  return (
    <div style={S.page}>
      ...
      <section style={S.panel}>
        ...
        {takes.length > 0 && (
          <>
            <div style={{ ...S.sliderHead, marginTop: 12 }}><span style={S.sliderLabel}>Take library</span></div>
            <div style={S.takeList}>
              {takes.map((t) => (
                <div key={t.id} style={{ ...S.takeRow, borderColor: t.id === activeTakeId ? "#E8A34C" : "#2C2540" }}>
                  ...
                  <button style={S.takeDel} onClick={() => deleteTake(t.id)}>✕</button>
                </div>
              ))}
            </div>
          </>
